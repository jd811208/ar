criaCartao(
    'Geografia',
    'Qual é a fonte de energia mais usada no brasil?',
    'Energia hidrelétrica'
)

criaCartao(
    'Língua Inglesa',
    'O que é do e does?',
    'são auxiliares usados para formar frases negativas e interrogativas e para enfatizar verbos no presente simples'
)
criaCartao(
    'Programação',
    'O que é o css',
    'é uma linguagem de programação usada para estilizar.'
)

criaCartao(
    'Resolução de Problemas',
    'Como se calcula o índice de cocção(IC)?',
    'IC= PC/PL'
)


criaCartao(
    'Projeto de Vida',
    'O que é Mercado Digital?',
    'O mercado digital refere-se a todas as atividades comerciais que ocorrem online, como compra, venda, publicidade, atendimento ao cliente e outras interações '
)

criaCartao(
    'Biotecnologia',
    'Qual é a célula presente nas plantas?',
    'Eucarionte vegetal'
)

criaCartao(
    'Robótica',
    'o que é um Circuito Elétrico?',
    'É um trajeto composto por vários componentes elétricos interconectados que produzem energia.'
)

criaCartao(
    'Educação Financeira',
    'O que é consumo consciente?',
    'É gastar o dinheiro de forma consciente evitando desperdícios'
)

criaCartao(
    'Química',
    'Qual é o ânodo da pilha?',
    'O elemento químico com menor carga'
)
criaCartao(
    'Sociologia',
    'Qual era o material de estudo de Karl Marx?',
    ' O materialismo histórico-dialético'
)

criaCartao(
    'Biologia',
    'Quais são as características dos peixes Ósseos?',
    'Possuem esqueleto ósseo, brânqueas internas e apresentam uma linha lateral'
)

criaCartao(
    'História',
    'Qual era o objetivo de Hittler ao assinar o Acordo de Não-Agresão?',
    'Era evitar uma guerra com a União Soviética após a invasão na polônia'
)

criaCartao(
    'Matemática',
    'Quanto é 8!?',
    '40320'
)

criaCartao(
    'Língua Portuguesa',
    'Quais são as características de um conto?',
    'Narrativa curta, poucos personagens e um espaço tempo menor'
)